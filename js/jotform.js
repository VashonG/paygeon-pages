        ...
        var ready = function () {
            ...
            try {
                ...
                HTMLFormElement.prototype.submit = (function (originalSubmitFn) {
                    return function () {
                        try {
                            // Set validatedRequiredFieldIDs to help identify the source of the submit event
                            if (!document.querySelector('[name="validatedNewRequiredFieldIDs"]')) {
                                var message = this.classList.contains('jotform-form') ? 'Submit method directly invoked' : 'Wrong form submitted - ' + this.name;

                                this.insert(new Element('input', {
                                    type: 'hidden',
                                    name: 'validatedNewRequiredFieldIDs'
                                }).putValue('Invalid Value: ' + message));
                            }

                            if (this.classList.contains('jotform-form')) {
                ...
                window.onpageshow = function (e) {
                    ...
                    if (e.persisted) {
                        ...
                        if (JotForm.browserIs.ios()) window.location.reload();
                        ...
                        // For safari browsers, remove files from file upload to prevent missing files in submissions
                            ...
                            $A(document.querySelectorAll('.qq-upload-success')).each(function (fileElement) {
                                var fileElementParent = fileElement.parentElement;

                                // Locally remove file from form and run cleanup/validation
                                JotForm.removeFile({
                                    fileElement: fileElement,
                                    fileName: fileElement.readAttribute('actual-filename'),
                ...
                if ($A(document.forms).length) {
                    ...
                    // Empty Submission Tracking: Do we still have any forms at this point?
                    trackExecution('forms-not-ready');
                }
...
//                        p.$jot('.jt-content, .jt-title').css('width', 'auto');
                    ...
                    onComplete: function (id, filename, response) {
                        ...
                        if (response.success) {
                            ...
                            if ($('draftID')) {
                                ...
                                if (!uploadedFile) {
                                    uploadedFile = new Element('input', {
                                        id: uploadedFileID,
                                        type: 'hidden',
                                        name: uploadedFileID
                                    });
                                    f.insert({ bottom: uploadedFile });
                                } else {
                                    uploadedFiles = uploadedFiles.concat(JSON.parse(uploadedFile.value));
                                }
                                uploadedFiles.push(filename);
                                uploadedFile.value = JSON.stringify(uploadedFiles);
                            }
                    ...
                    onDelete: function(folder, field, filename) {
                        if ($('draftID')) {
                            ...
                            if (uplodedFileArray.includes(filename)){
                                ...
                                var deletedFile = $(deletedFileID);
                                if (!deletedFile) {
                                    deletedFile = new Element('input', {
                                        id: deletedFileID,
                                        type: 'hidden',
                                        name: deletedFileID
                                    });
                                    f.insert({ bottom: deletedFile });
                                } else {
                                    deletedFiles = deletedFiles.concat(JSON.parse(deletedFile.value));
                                }
                                deletedFiles.push(filename);
                                deletedFile.value = JSON.stringify(deletedFiles);
                            }
                        }
                    ...
                    if (!matchForThisResult[condition.action[0].resultField+subfield]) {
                        try {
                            ...
                            if(document.getElementById('draftID')) {
                                var defaultValue = "input_"+toBeClearField in JotForm.defaultValues ? JotForm.defaultValues["input_"+toBeClearField] : "";
                                $('input_' + toBeClearField).value = defaultValue;
                                $('input_' + toBeClearField).triggerEvent('keyup');
    ...
    */
        ...
        $A(options).each(function(option){
            option.observe('click', function(event){
                ...
                if (selectedOptions[0].value === 'All' || (JotForm.isVisible(dropdownHint) && selectedOptions[0].value === 'All')) {
                    selectedValueArea.addClassName('all_selected');
                } else if (selectedValueArea.hasClassName('all_selected')) {
                    selectedValueArea.removeClassName('all_selected');
                }
    ...
    setCustomPriceSource: function (pairKey) {
        if (pairKey) {
            var customFirstPaymentPriceData = $(pairKey + '_custom_first_payment_price');
            var customPriceSourceData = $(pairKey + '_custom_price');

            if (customFirstPaymentPriceData && customFirstPaymentPriceData.hasAttribute('data-price-source')) {
                var firstCustomPriceSourceKey = customFirstPaymentPriceData.getAttribute('data-price-source');
                if (firstCustomPriceSourceKey) {
                    var firstCustomPriceSourceInput = document.getElementById('input_' + firstCustomPriceSourceKey);
                    if (firstCustomPriceSourceInput) {
                        firstCustomPriceSourceInput.value = customFirstPaymentPriceData.getValue();
                    }
                }
            }
            ...
            if (customPriceSourceData && customPriceSourceData.hasAttribute('data-price-source')) {
                ...
                if (customPriceSourceKey) {
                    var customPriceSourceInput = document.getElementById('input_' + customPriceSourceKey);
                    if (customPriceSourceInput) {
                        customPriceSourceInput.value = customPriceSourceData.getValue();
                    }
                }
            }
        }
    ...
    */
    ...
    handleCoupon: function () {
        ...
        if ($('coupon-button')) {
            ...
            cb.innerHTML = couponButtonMessage;
            var formID = $$('input[name="formID"]')[0].value;
                ...
                if (ci.value) {
                    ...
                    JotForm._xdr(JotForm.getAPIEndpoint() + '/payment/checkcoupon', 'POST', JotForm.serialize({
                        ...
                        try{
                            ...
                            cl.hide();
                            cb.show();
                            couponMessage = $this.paymentTexts.couponValid;
                            if (typeof FormTranslation !== 'undefined') {
                            ...
                            if (response.content === "expired") {
                                couponMessage = $this.paymentTexts.couponExpired;
                            } else if (response.content === "ratelimiter") {
                                couponMessage = $this.paymentTexts.couponRatelimiter;
                            } else {
                                couponMessage = $this.paymentTexts.couponInvalid;
                            }
                            if (typeof FormTranslation !== 'undefined') {
                                couponMessage = FormTranslation.translate(couponMessage);
                            }
                    ...
                    couponMessage = $this.paymentTexts.couponBlank;
                    if (typeof FormTranslation !== 'undefined') {
                        couponMessage = FormTranslation.translate(couponMessage);
                    }
    ...
    setPagePositionForError: function () {
        ...
        if(firstError) {
            ...
            if (!JotForm.noJump && erroredLine) {
                erroredLine.scrollIntoView({ block: 'center', behavior: 'smooth' });
  ...
  */
  handleSignSignatureInputs: function() {
    ...
    validateAll: function (form, scopeSelector) {
        function handleValidateAll(form, scopeSelector) {
            ...
            var ret = true;
            JotForm.validatedRequiredFieldIDs = JotForm.validatedRequiredFieldIDs === null || typeof JotForm.validatedRequiredFieldIDs !== 'object' ? {} : JotForm.validatedRequiredFieldIDs;
            ...
            var paymentInCurrentSection = JotForm.currentSection && JotForm.currentSection.down('li[data-payment="true"]');
            ...
            if ((paymentInCurrentSection || noPage) && JotForm.browserIs.firefox()) {
                ...
                if (requiredProductInputs.length > 0) {
                    for (var i = 0; i < requiredProductInputs.length; i++) {
                        if (!requiredProductInputs[i].checked) {
                            ret = false;
                            break;
                        }
                    }
                ...
                try {
                    ...
                    if (JotForm.validatedRequiredFieldIDs != null && typeof JotForm.validatedRequiredFieldIDs === 'object') {
                        ...
                        form.insert(new Element('input', {
                            type: 'hidden',
                            name: 'validatedNewRequiredFieldIDs'
                        }).putValue('Invalid Value: ' + typeof JotForm.validatedRequiredFieldIDs));
                    }
  ...
  /**
    ...
    removeFileForMobile: function (options) {
        ...
        var isDeleted = false;
        var fileArray = window.filesForMobile[options.qid];
    ...
    removeFile: function (options) {
        ...
        var qid = options.fileElement.up('.form-line').id.split('_')[1];
        ...
        if (options.hiddenTempUploadInput) options.hiddenTempUploadInput.remove();
    }
};
...
function trackExecution(value) {
    var jsExecutionTrackerElement = document.getElementById('jsExecutionTracker');
